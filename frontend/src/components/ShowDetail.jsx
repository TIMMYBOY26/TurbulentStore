...
const ShowDetail = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > swipeThreshold) {
      setCurrentImageIndex((prev) =>
        prev === show.image.length - 1 ? 0 : prev + 1,
      );
    } else if (distance < -swipeThreshold) {
      setCurrentImageIndex((prev) =>
        prev === 0 ? show.image.length - 1 : prev - 1,
      );
    }
    setStartX(0);
    setEndX(0);
  ...
  if (loading)
    return (
      <p className="text-center text-xl pt-20 font-black uppercase tracking-widest">
        Loading...
      </p>
    );
  if (error)
    return (
      <p className="text-center text-red-500 pt-20 font-bold">Error: {error}</p>
    );
  if (!show)
    return <p className="text-center pt-20 font-black">Show not found</p>;
  ...
  // API CONTROL LOGIC
  const isTicketActive = show.isTicketAvailable || false;
  const ticketLabel = show.ticketStatus || "Coming Soon";
  const isSoldOut = ticketLabel === "Sold Out";
          ...
          >
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
              {show.image.map((_, i) => (
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all ${currentImageIndex === i ? "bg-black w-4" : "bg-gray-300 w-1"}`}
                />
              ))}
            </div>
          </div>

          {/* Desktop Stack Display */}
          <div className="hidden lg:flex flex-col items-center">
            {show.image.map((img, index) => (
              <div
                key={index}
                className="group w-full max-w-md bg-gray-50 rounded-lg overflow-hidden mb-4 shadow-sm cursor-zoom-in"
                onClick={() => setSelectedImg(img)}
              >
                <img
                  src={img}
        ...
        <div className="lg:w-1/2 w-full text-left">
          ...
          <div className="space-y-3 mb-6 py-5 border-t border-b border-gray-100">
            <p className="text-sm flex items-baseline">
              <span className="text-gray-400 uppercase tracking-widest text-[10px] mr-3 w-20 flex-shrink-0 font-black">
                Location:
              </span>
          ...
          <div className="flex flex-col space-y-2 max-w-xs">
            {show.instagramLink && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-center bg-blue-600 text-white font-black py-3 text-[10px] uppercase tracking-widest hover:bg-blue-700 transition shadow-sm"
              >
                {instagramButtonText}
              </a>
            )}
            ...
            <button
              ...
              disabled={!isTicketActive || isSoldOut}
              className={`text-center font-black py-3 text-[10px] uppercase tracking-widest transition-all shadow-lg active:scale-95
                ${
                  isTicketActive && !isSoldOut
            ...
            <button
              onClick={() => navigate(-1)}
              className="text-center bg-gray-100 text-gray-400 font-black py-2.5 text-[9px] hover:bg-gray-200 transition uppercase tracking-widest mt-4"
            >
              ← Back
            </button>
          </div>
