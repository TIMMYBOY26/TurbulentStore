    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl text-center animate-in fade-in zoom-in duration-300">
        ...
        <button
          ...
          className="w-full py-4 bg-black text-white font-bold rounded-2xl hover:bg-gray-800 transition-all shadow-lg"
        >
          查看我的訂單
        </button>
      </div>
...
const PlaceOrder = () => {
  ...
  // 清除收據的輔助邏輯：切換到面交現付時，自動清空可能誤傳的收據
  useEffect(() => {
    if (method === "cod") {
  ...
  // 判斷當前付款方式是否必須上傳收據
  const isReceiptRequired = method !== "cod";
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const totalAmount =
        ...
        (deliveryType === "sf" ? Number(delivery_fee) : 0);
      ...
      >
                ...
                {deliveryType === "sf" && (
                  ...
                  >
                    <PaymentOption
                      ...
                      label="PayMe"
                      qr={assets.paymeCode}
                      instructions={<p>1. 掃描下方 QR Code 付款</p>}
                    />
                    <PaymentOption
                      id="fps"
                      label="轉數快 (FPS)"
        ...
        <div className="lg:w-[380px]">
          <div className="bg-white border border-gray-100 shadow-2xl rounded-[2.5rem] p-8 sticky top-10 text-center">
            ...
            {/* 動態提示文字 */}
            {isReceiptRequired && !receiptImage && (
              <p className="text-red-500 text-xs font-semibold mt-3 animate-pulse">
                ※ 請先在上傳付款截圖以解鎖下單
              </p>
            )}
