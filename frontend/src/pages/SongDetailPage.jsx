...
const SongDetailPage = () => {
  ...
  useEffect(() => {
    const fetchSong = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/song/${id}`);
        setSong(response.data.song);
      } catch (err) {
        setError(err.message);
      } finally {
        // Add a small delay for a smooth animation transition
        setTimeout(() => {
          setLoading(false);
        }, 800);
      }
  ...
  if (!loading && !song)
    return <p className="text-center pt-20">No song found</p>;
  ...
  return (
    <>
      ...
      {/* PAGE CONTENT */}
      <div
        className={`song-detail-page transition-opacity duration-1000 ${
