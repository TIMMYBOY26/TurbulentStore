...
const ShowDetailPage = () => {
  ...
  if (!loading && !show) return <p className="text-center pt-20">No show found</p>;

  return (
    <>
      {/* Wave Loader remains unchanged */}
      {loading && (
        <div className="fixed top-[80px] bottom-0 left-0 right-0 z-40 flex flex-col items-center justify-center bg-white">
          ...
          <style>{`@keyframes wave { 0%, 100% { height: 1.5rem; transform: translateY(0); } 50% { height: 4rem; transform: translateY(-5px); } }`}</style>
        </div>
      )}

      {/* FIXED PAGE CONTENT WRAPPER */}
      <div
        className={`show-detail-page w-full flex flex-col items-center p-4 md:p-8 transition-opacity duration-1000 ${loading ? "opacity-0" : "opacity-100"
          }`}
      >
        <div className="max-w-6xl w-full"> {/* Container to prevent horizontal stretching on ultrawide screens */}
